  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kita Systems AO"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kita Systems AO" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest ao-entity-missing-is-held-and-unoverridable
  (testing "missing Japan-resident authorized representative -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:ao-entity-missing} (-> (store/ledger db) last :basis)))
