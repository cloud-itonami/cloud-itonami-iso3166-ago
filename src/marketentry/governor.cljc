(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Angola-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Angola-resident-rep missing -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ao-entity?
                                       true` (near-universal for Angola
                                       ...
                                       Grounded in Angolan legal entity
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NIF/統一資格/SNCP e-procurement登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ao-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ao-entity? true`, INDEPENDENTLY verify
  `:has-ao-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ao-entity?` ground truth (most Angola public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ao-entity? e))
                 (not (true? (:has-ao-entity? e))))
        [{:rule :ao-entity-missing
          :detail (str subject " はアンゴラ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ao-entity-missing-violations request st)
