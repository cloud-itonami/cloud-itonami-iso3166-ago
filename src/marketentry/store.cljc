(ns marketentry.store
  "SSoT for the AGO market-entry compliance actor, behind a `Store`
  ...
  every prior cloud-itonami actor in this fleet uses.
    ...
    - `DatomicStore` -- backed by `langchain.db`, a Datomic-API-compatible
                        EAV store.
    - `KotobaseStore` -- the durable one the Worker uses, over the host's
                        injected database api. Lives HERE rather than in an
                        edge namespace because `commit-record!` needs
                        `draft-filing!` / `submit-filing!`, which are private
                        to this file.
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems AO" :portal "SNCP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ao-entity? true :has-ao-entity? true
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AGO" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "SNCP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ao-entity? true :has-ao-entity? true
             :requires-nif? true :nif-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "SNCP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "AGO" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "SNCP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ao-entity? true :has-ao-entity? false
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "AGO" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "SNCP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-ao-entity? true :has-ao-entity? true
             ...
             :drafted? false :submitted? false
             :jurisdiction "AGO" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "SNCP e-procurement"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-ao-entity? has-ao-entity?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-ao-entity?)  (assoc :engagement/requires-ao-entity? requires-ao-entity?)
    (some? has-ao-entity?)       (assoc :engagement/has-ao-entity? has-ao-entity?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-ao-entity? :engagement/has-ao-entity?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-ao-entity? (boolean (:engagement/requires-ao-entity? m))
     :has-ao-entity? (boolean (:engagement/has-ao-entity? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-ago"})
